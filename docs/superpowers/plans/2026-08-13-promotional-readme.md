# Promotional README Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Replace the minimal public README with a credible promotional landing page that converts overlapping transcript users and Codex/GitHub users into trial, Star, and sharing actions.

**Architecture:** Keep `SKILL.md` and runtime behavior unchanged. Encode the non-negotiable README claims in the existing PowerShell static contract, then replace `README.md` with one Chinese landing page whose first line is fixed and whose technical claims stay within verified Windows behavior.

**Tech Stack:** GitHub-flavored Markdown, Windows PowerShell 5.1 static contract, existing Python `unittest` and Skill validators.

---

### Task 1: Specify the promotional README contract
...
Immediately after `$readme = Get-Content ...`, add:
...
Expected: FAIL with `README first line must be exact`.

### Task 2: Replace the README with the promotional landing page

**Files:**
- Modify: `README.md`

- [ ] **Step 1: Replace `README.md` with the exact approved content**
...
- [ ] **Step 2: Run the static contract and verify GREEN**
...
- [ ] **Step 3: Inspect the rendered content mechanically**
...
required_first_line = "\u70b9\u70b9\u5173\u6ce8\u8c22\u8c22\u55b5"
assert lines[0] == required_first_line
assert lines.count(required_first_line) == 1
opened = False
for line in lines:
    if line.startswith("```"):
        opened = not opened
assert not opened, "unclosed Markdown code fence"
assert len(text) >= 6000
print(f"README inspection: PASS ({len(lines)} lines, {len(text)} characters)")
'@ | python -X utf8 -
```

Expected: first-line and length assertions pass. If the fence-count assertion does not model the Markdown correctly, replace it with a stack-based fence scan before changing README content.

- [ ] **Step 4: Commit the README upgrade**

```powershell
git add README.md tests/static-contract.ps1
git commit -m "docs: expand promotional readme"
```
...
**Files:**
- Verify: `README.md`
- Verify: `tests/static-contract.ps1`
...
Expected: 44 Python tests, parity contract, static contract, five remote assets, and both Skill validators pass; `git diff --check` prints no errors.
...
git diff main...HEAD -- README.md tests/static-contract.ps1
```

Expected: only the intended README, contract, design, and plan changes appear.

- [ ] **Step 3: Fast-forward local main and re-run the static contract**

From the primary repository, fetch `origin/main`, confirm the worktree is clean, fast-forward `main` to `codex/promotional-readme`, then run:

```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File tests/static-contract.ps1
```

Expected: `static Skill contract: PASS` on merged `main`.

- [ ] **Step 4: Push and confirm GitHub Actions**

```powershell
git push origin main
```

Wait for the workflow attached to the pushed commit. Expected: `windows-contracts` concludes `success`; if it fails, diagnose and fix before reporting completion.
